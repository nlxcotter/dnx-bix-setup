...
**G — Per-project channels / hub-and-spoke / section-ownership / cc-on-exception.** Comms is NOT one universal board (that invites data collisions). Each project has its OWN gated channel to the GM (`comms/<project>.md`: `tcs.md`, `nlxa.md`, a future `nlxsys.md`). **Hub-and-spoke:** the GM is the hub; every PM channel flows up to the GM; each PM works ONLY its own channel. **This is a communication-management gate, not secrecy/clearance** — it keeps each project's traffic in one clean lane and avoids collisions; projects are not hidden from each other. **Cross-project Q&A is RELAYED and moderated by the GM** (the only seat that reads across channels), so the flow stays orderly. Within a channel: each session edits **only its own status row** and **appends its own thread entries** (newest first), never rewriting the other's. The substantive records (`build-progress.md`, `tcs-rebuild-cs.md`, `airi-progress-report.md`) are **strictly single-writer.** cc only when the other **must act or know.** *(Supersedes the earlier "single universal channel" framing, on Cotter's directive 2026-07-16.)*
