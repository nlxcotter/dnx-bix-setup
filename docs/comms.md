...
|-----------|-------------|----------|---------|
| TCS (PM)  | **6/29 session (DEPLOYED; git-commit batch pending Cotter's go):** review-request **email + per-client "Review" button** (star-routed 4 to 5★ to Google review, 1 to 3★ to private support@); **TCS Clients** width pass (SMS rename, email truncate), **Delete removed from the list** (contact-detail only, per Cotter), and **clickable SMS / AV / Sent checkmarks** (each writes the real tag, sends no email, fires the one-way Google re-sync). **Laptop Battery Replacement** service + page + conditioning blog §6; **Remote Support** 3rd check-in type w/ internet gate; sitewide **Bing SEO** title/description fixes; **IndexNow** auto-submit wired (HTTP 202/200); repair-details now a dated, newest-first, service-stamped append log. Earlier this week: Google Contacts one-way sync LIVE + gnat-proof teaching standard. | Cotter: **greenlight the git-commit batch**; decide whether SMS/AV *removal* should also strip the Google label (currently additive-only, label lingers) | none |
| AIRI (GM) | Synced to PM's 6/29 catch-up. **Standing discipline adopted: reconcile comms against git + build-progress BEFORE any status report** (board lagged reality → I nearly re-proposed already-shipped review-automation). **DNS-302 CLOSED** (PM + my external check agree); gnat-standard fold = next GM action | Cotter: greenlight PM commit batch; SMS/AV label-removal call · GM: do gnat-standard cross-link | none |
