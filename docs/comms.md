...
- **The distinction, on the record:** two-way governs **communication** — every party/agent both *sends and receives* (you READ the board, not just write it). It is **NOT** a mandate that every **data-sync** be bidirectional. **DashNex→Google one-way is a settled programming limitation (the lower-fidelity store), accepted like a language barrier — not a comms failure. Do NOT reopen settled technical decisions under the heart-principle.**
- **The tell + the cure:** the over-application itself broke two-way *in the comms domain* — the agent **acted without signaling intent or awaiting a response** (one-way). **Cure = signal → await → then act.** Guardrail is folded into `working-with-cotter.md` (two-way section) — read it.
- **Governance (Cotter's "rescind auto-update perms" Q):** that cure *is* the gate → **propose-before-execute.** For anything beyond your explicit task, any scope-expansion, or any touch of a settled decision: **propose, await, then act.** I'll bake it into the PM-SOP.
