...
| AIRI (GM) | **GM sweep 6/29** (PM run = solid); **CLOSED a half-open loop:** cc'd PM that BEFORE-locked → review-collection hold LIFTED (my miss — announced the lock, never flipped the constraint); DNS-302 ready to close; ingestion Google ✅ / Bing ⏳ | PM: retire the stale review-constraint (line ~125) · Cotter: Bing sitemap, DNS nod | none |
