...
### 2026-08-07 → 08-11 — Wellness content, a science correction from the owner, and a cross-link audit
- Phase·Step: AFTER / content depth, and the owner as fact-checker
- What: Drafted a screen-time-and-sleep post, held it **unpublished** at the owner's instruction, then rewrote it after he challenged the science. Ran a **cross-link audit of all 37 published posts**: zero broken links, but **8 orphans with no inbound links at all** and **15 relying on the Related Reading block with no in-prose links out**.
- Why: The post sells nothing. The owner's rule, stated plainly after the PM twice asked him to "rule on" whether it belonged: *"It's called added value... When I add value that people can find, it builds trust, trust builds relationships, relationships earn revenue."* The PM had been applying a conversion filter to a content decision.
- Decision(s): The only editorial test is **"does this add value, or is it just noise?"**, never "does this convert."
- Lesson / adjustment: **The owner caught a factual error the PM had written with confidence.** The draft said the brain *learns* to associate light with wakefulness; the real mechanism is hardwired melatonin suppression. His own source also showed night-mode and blue-light glasses lack support, contradicting the draft. **The same error was already live in an earlier post and had to be corrected there too.** Second lesson from the same stretch: **a post-deploy check without a cache-buster is not a check** — one run against a stale edge copy reported a freshly-deployed paragraph as missing.
- Artifacts: `webapp/src/app/blog/screen-time-blue-light-and-sleep/page.tsx`, `webapp/src/app/blog/back-to-school-family-tech-checklist/page.tsx`

### 2026-08-11 → 08-13 — A field the platform does not have, and a push that would have downgraded production
- Phase·Step: AFTER / CRM fit, and a near-miss worth more than the feature
- What: Added **Company / Business Name** to the TCS Clients module: searchable, sorted, displayed as the primary line for business clients, and mapped to Google Contacts' native `organizations[]`.
- Why: DashNex's contact record **has no company column**. Eighteen columns, none of them organisation, confirmed by reading the 1.6.6 schema directly. The only place the word appears in the whole module is a placeholder in DashNex's own custom-fields screen reading "e.g. Company Name", which is the vendor's own example of what belongs in a custom field.
- Decision(s): Self-creating field definition, so the owner never builds it in a dashboard. **Company as the primary display line**, per his rule that a business IS its business name and *"my clients are NOT their computers."* Google mapping **strictly additive**, writing an organisation only where Google has none.
- Lesson / adjustment: **`dashnex app push` refuses on remote drift, and `--force` past it can downgrade a live site.** Local `package.json` was months behind (`core` 1.3.0 vs 1.3.7, `auth` 1.2.3 vs 1.4.0) and still listed two modules the owner had removed via the dashboard. **Forcing would have downgraded every module on a live business site and reinstated two deliberately deleted ones.** The trap: copying the server's file byte-for-byte does **not** clear the block, because the comparison runs against a manifest cache that only refreshes on an **in-place** `dashnex app pull .`. *On a divergence the server is usually the correct side. Read the diff before reaching for `--force`.*
- Artifacts: `webapp/src/site/admin/company-field.ts`, `webapp/src/site/admin/ClientsPage.tsx`, `webapp/src/site/google-sync.ts`; commits `4c22cde`, `6960567`, `6337f14`.

### 2026-08-11 — Reading the headers instead of the alarm: a spoof that was not a spoof
- Phase·Step: AFTER / incident triage
- What: A scam message reached the owner carrying his own `noreply@` address. Read the full headers: `dkim=pass`, `spf=pass`, an `X-Original-Message-ID` ending `@mail.gmail.com`, and a `Received: … with HTTP` hop. **Legitimate forwarding of a real Gmail message, not a forged sender.**
- Why: His first read was that his domain had been abused, and he said plainly that he panicked a little.
- Decision(s): Say the domain was not spoofed, and say it before proposing any hardening.
- Lesson / adjustment: **Name the event, not the category it rhymes with.** The practical consequence mattered more than the reassurance: the DMARC work already drafted **would not have stopped this message**, and selling it as the fix would have left him believing his domain had been abused. A separate finding from the same read, a live Dropbox share link published in a public DNS TXT record, was real and he removed it.
- Artifacts: `tcs/techdesk/handoffs/2026-08-11-email-auth-findings.md`

### 2026-08-18 → 08-19 — Every URL returned 200, and the fix that would have taken down the admin
- Phase·Step: AFTER / SEO integrity on a platform you do not control
- What: Found that **every URL on the site answered 200**. `/blog/no-such-post`, `/services/no-such-service`, anything at all. Google could index an unbounded set of pages that do not exist, all with identical titles. Shipped real 404s under `/blog` and `/services` plus `noindex` on the root catch-all, and retired a ghost `tech-services` page that had been serving an empty shell for months.
- Why: Same class of thin-content problem Bing flagged in July, with no ceiling on it.
- Decision(s): **Correct status only where provably safe; `noindex` everywhere else.** `/blog` and `/services` are wholly ours, every entry is a static directory, and static beats dynamic in the router, so a catch-all there can only ever catch a genuine miss.
- Lesson / adjustment: **The root cause is the platform's, and the obvious fix would have broken the site.** `AuthPage` is a **client** component: it returns 200 with a loading spinner and only decides 404 after hydration, so the status is settled before the page lookup runs. A server-side gate was impossible because **`dashnex router` reports "0 pages from 14 modules"** — module pages register in `src/client.ts`, never `src/server.ts`, so the server cannot know whether `/admin/clients` is real. Gating there would have 404'd the entire admin and client portal; a hardcoded path list would have held until the vendor added a page, then failed silently. **Indexing was the harm. The status code was not.**
- Artifacts: `webapp/src/app/blog/[...rest]/page.tsx`, `webapp/src/app/services/[...rest]/page.tsx`, `webapp/src/app/[...path]/page.tsx`; commit `1ef342d`.

### 2026-08-17 → 08-21 — Review requests: a robot sender, and telemetry that was our own bug
- Phase·Step: AFTER / the highest-leverage growth lever in the business
- What: The owner asks every client for a review in person, gets a verbal yes, and sends a request. Conversion felt like zero. **Read the live database directly** (`dashnex db execute --remote`, no admin session required) and found **0 opens in 231 outgoing emails**, **1 click in 231**, and **159 rows stuck at `pending`**. Also found the requests were sending from **`noreply@` with no sender name**.
- Why: Reviews are the one signal that is both permanent and movable. Count went **3 at baseline to 14 on Google** across this period. Everything else the site does is rented rank; reviews are owned.
- Decision(s): Send from a named, replyable human address, **"Cotter at The Computer Solution"**, with every other email left on `noreply@`. A client agrees face to face and then hears from a robot, and the relationship the ask was leaning on is gone at the moment it had to carry the request.
- Lesson / adjustment: **★ The PM spent a long stretch rewriting the email copy while the data sat one CLI query away. Read the data before rewriting the words.** Two further corrections came from the owner: the star-ladder convention he had copied from a vendor build **works**, and the PM had its friction backwards — a star row is *lower* friction than a button, because clicking a star is a twitch and "leave a review" is a chore. **And the platform bug was ours:** vendor support identified that `getMailer().send()` is the low-level transport that knows nothing about tracking, while `sendTransactional()` assigns the email id, rewrites links, injects the open pixel and wires delivery status. Clicks recorded immediately; opens needed `core` 1.3.8. **The system had been converting the whole time and nobody could see it.**
- Artifacts: `webapp/src/app/api/admin/review-request/route.ts`, `webapp/src/site/review-email.ts`, `tcs/docs/support/dashnex-email-tracking-bugcheck.txt`; commits `1f5c7ee`, `bc48f49`, `5a032d3`.

### 2026-08-23 — Publishing a real incident: four corrections between draft and ship
- Phase·Step: AFTER / turning field work into content nobody else in the market has
- What: Shipped **"Five Antivirus Programs Said This Computer Was Clean. It Wasn't."** from two real local infections, fully anonymised. Fileless malware running through legitimate signed software, payload only ever real in memory, phoning home every thirty seconds and executing whatever it was handed, live. Both clients hit by the **same commercially-built kit**.
- Why: A genuine local incident nobody else in the market can write about, and the owner's own instinct that this is the shift worth naming.
- Decision(s): **Frame antivirus as necessary but not sufficient**, never as useless, because the owner sells and sets up antivirus and a bashing piece shoots his own foot. **No product named**, his call. **Concept, not recipe:** no file names, folder names, task names, server addresses or builder IDs.
- Lesson / adjustment: **Four things were caught between draft and publish, and in three of them the wrong version was the more quotable one.** (1) The owner killed an analogy as *talking down to the reader rather than educating*. (2) The Tech Desk's "ten-year lease on her machine" was **false** and they owned it; reframed as a commercial licence valid ten years, evidence of an industry rather than victim-targeting, and it lands harder for being true. (3) They flagged an April timeline as unsupported, then **rechecked the samples against their own writeup and retracted their own catch** — April was real, and the recheck produced a better story: compromised in April and **re-tooled repeatedly all summer**. (4) The PM wrote that it leaves no trace at shutdown, which was wrong **and contradicted the article's own ending**. **The transferable lesson: a fact-check is only worth something if the checker is willing to be wrong out loud, twice.**
- Artifacts: `webapp/src/app/blog/malware-your-antivirus-cant-see/page.tsx`; commits `2c6330b`, `41f6132`.

### 2026-09-04 → 09-06 — The bench supplies the story, and a client's search history supplies the gap
- Phase·Step: AFTER / the business feeding the content instead of the other way round
- What: Shipped **"What Does It Actually Mean to Have a Data Backup?"**, added **Mac coverage to the virus-removal service page**, and built the **intake confirmation email** ("I have your machine, the service has begun") sendable by the owner OR by the Tech Desk on their existing key.
- Why: Three things arrived from the work itself rather than from a content calendar. **A machine in pieces on the bench**: dropped, broken charging port, that was the owner's whole concern; twelve years old with a mechanical drive; the drive did not survive the drop, and the owner had to call the next day and say the computer was totaled and very likely everything on it. **They did not come in worried about their files. They came in about a charging port.** And **a real search query out of a client's history**, handed over verbatim: *"Is there anyone in Durango, CO who can run a diagnostic test on my MacBook Pro to discover Malware?"*
- Decision(s): **Open the post on the bench job rather than on the idea.** And on the Mac gap, **concede the prevailing advice rather than argue with it**: Google currently tells Mac owners that XProtect and Gatekeeper are good and third-party security costs performance. All true. The FAQ agrees with every word and then says it answers a different question, because built-in or bought, it all works by recognizing known bad software, and the local cases used none.
- Lesson / adjustment: **★ THE OWNER IS THE FACT-CHECKER AND HE CAUGHT A FABRICATION.** I wrote that the drive failed from age. He never told me a cause; I filled the gap and stated it as fact. It was the drop, and that is the stronger story anyway. He also killed *"the files were fine right up until the moment somebody looked"* as **categorically incorrect** — they were not fine, they were already gone, and discovery did not kill them. **Second lesson, on process rather than copy: a routine that lives partly in your head is a routine you will run wrong.** The documented ship sequence had six steps; three more were memory-only, and two of those were dropped on a ship where the owner had named them explicitly in the same sentence. Written down now, nine steps. **Third: a page can be silent exactly where the fear is.** The site said "Mac" clearly on repair costs, screens and batteries, and never once on the malware pages. Nobody notices an absence.
- Artifacts: `webapp/src/app/blog/what-is-a-data-backup/page.tsx`, `webapp/src/app/services/virus-removal/page.tsx`, `webapp/src/site/intake-email.ts`, `webapp/src/site/admin/send-intake-notice.ts`; commits `084bf3e`, `824cabe`.

### 2026-09-09 — Print turned out to be a site-wide surface nobody had ever looked at
- Phase·Step: Content & utility / build a free tool, then discover the platform fault it exposes
- What: Built `/printer-test-page` (a real printer test pattern: color bars, grayscale ramp, hairlines, type at 6-24pt, a margin registration mark) plus a hosted PDF, then shipped the blog post that feeds it, *"Your Printer Says It's Offline. It Probably Isn't."* **The test page is what found the bug.** Printing it produced the site header, the footer sitemap, the chat bubble, in landscape, clipped on the right, across two sheets. **Four faults, three of them affecting every page on the site.** (1) `print:hidden` applied to page sections never reaches the chrome, because `SiteHeader`/`SiteFooter`/`MobileCallBar` sit outside `{children}` in `SiteShell`. (2) The InstraChat bubble is injected `position: fixed` outside the React tree, so no utility class can reach it and it needs a real `@media print` rule. (3) `min-h-screen` is `100vh`, a full forced sheet in print, so short pages spill a blank second page. (4) `print:max-w-none` lets the page lay out at the **browser viewport** width rather than the paper width. Then a fifth on the next cycle: `@page { margin }` is overridden by the print dialog's own Margins setting, so the page printed flush into the top-left corner with the registration mark clipped off.
- Why: A free tool aimed at a real search gap ("printer test page" is getting harder to find online, per the owner). The AIO play is to own the utility, not just write about it. The print stylesheet was never the point and became the whole job.
- Decision(s): Fix it **site-wide in `SiteShell` and `tailwind.css`, not on the one page**, because every page had been printing its own nav bar. Pin print layout to real paper inches (`@page { size: letter portrait }`, `html, body { width: auto }`), and **make margins padding we own** rather than `@page` margins a dialog can override. Rebuild the "fine lines" test as a genuine 1px-on-1px gradient after the first version printed as fat blocks and tested nothing. Blog CTA points at `/services/home-office-setup` at the flat **$127** in the owner's own framing (fifteen minutes or a full hour, no clock running), with remote stated **conditionally** because a printer that has dropped off the network needs him on site.
- Lesson / adjustment: **★ A stylesheet nobody has printed is untested code, and a print bug reproduces differently on every screen size.** Fault (4) would have looked nearly correct on a laptop; it only became landscape-and-clipped on the owner's 28" 4K monitor, because the browser was laying out at ~3800px and trying to cram that onto a sheet. **Verify print on the widest screen available, not the developer's.** Three test cycles with the owner, each one surfacing a real and different defect, none of it guessable from the screen rendering. Second lesson, cheaper: the US-English gate returned CLEAN while `colour` sat twice in a source comment written an hour earlier, because the word was not in the pattern. **A pattern is only as good as its last miss; a clean result is weaker evidence than it feels like.**
- Artifacts: `webapp/src/app/printer-test-page/page.tsx` · `webapp/public/printer-test-page.pdf` · `webapp/src/app/blog/printer-says-offline/page.tsx` · `webapp/src/site/components/SiteShell.tsx` · `webapp/src/styles/tailwind.css` · commits `1031497`, `73c8ab0`
