...
**⛔ SUPERSEDED IN PART, 2026-08-05, Cotter's ruling. This method's checkout step no longer describes the house standard.** His words: *"GM insisted that I maintain the basic checkouts (for purposes of brevity and simplicity) and I accepted that, and then I discovered the customized checkouts and I was no longer satisfied with 'good enough' so I wanted to make sure that in this build they were customized to MY PREFERENCES."* **CUSTOMIZED checkouts are now the standard. The default checkout is the fallback, not the recommendation**, and the bump is a designed part of the funnel rather than a later add-on. The four builds below are still correct mechanics. **Step 4 is not.** Anything derived from this method, including the whitelabel README, inherits the stale premise unless it is read with this note.
...
**The catch:** `npm run instrachat-gap` in the webapp diffs the live sitemap against the source table, prints any page the bot cannot see, and **exits non-zero** so a ship checklist treats it as a stop rather than a note. Ship-routine step 8. Source: `webapp/scripts/instrachat-gap.mjs`. **Why it earns a line next to the warning above it:** that warning was written in July and the gap happened anyway, on the same site the warning came from. 40 posts live, 33 known to the bot, **7 invisible, the oldest for seven weeks**, and nothing surfaced it until a passing question did. The warning was not wrong, it was **unconnected to anything that acts.** A trap taught without its catch is half a lesson, and the omitted half is the half that failed.
...
**Follow-up answered (DashNex support, 2026-07-29):** also **NOT possible under the new DashNex ID system**, and the Dev Team noted **no future plan** for it. So no-transfer is the settled state across all current DashNex surfaces. Cotter accepts it for now, hoping it changes down the line, but design as if it never will.
